...
_CSV_HEADERS = ['ods_id', '_type', 'uuid', 'inCollection']
...
class ODSDataspotMapping:
    ...
    def __init__(self, database_name: str):
        ...
        # Mapping: Dict[str, Tuple[str, str, Optional[str]]] -> Dict[ods_id, (_type, uuid, inCollection)]
        ...
        try:
            with open(self.csv_file_path, 'r', newline='', encoding='utf-8') as csvfile:
                ...
                for row in reader:
                    ...
                    if not all(h in row for h in ['ods_id', '_type', 'uuid']):
                         self.logger.warning(f"Skipping row due to missing required columns (ods_id, _type, uuid): {row}")
                         continue
                    ...
                    self.mapping[ods_id] = (_type, uuid_val, inCollection)
    ...
    def _save_mapping(self) -> None:
        ...
        try:
            ...
            with open(self.csv_file_path, 'w', newline='', encoding='utf-8') as csvfile:
                ...
                for ods_id, (_type, uuid_val, inCollection) in sorted_mapping:
                    # Ensure inCollection is written as empty string if None
                    writer.writerow([ods_id, _type, uuid_val, inCollection or ''])
        except (IOError, PermissionError) as e:
    ...
    def get_entry(self, ods_id: str) -> Optional[Tuple[str, str, Optional[str]]]:
        """
        Get the _type, UUID, and inCollection for an ODS ID if it exists.
    ...
    def add_entry(self, ods_id: str, _type: str, uuid_str: str, in_collection: Optional[str] = None) -> bool:
        """
        ...
        if empty_params:
            ...
            self.logger.warning("Provided values - ods_id: '%s', _type: '%s', uuid_str: '%s'",
                             ods_id, _type, uuid_str)
            return False

        # Validate UUID format
        if not self._is_valid_uuid(uuid_str):
            self.logger.warning("Invalid UUID format: '%s' for ods_id '%s'", uuid_str, ods_id)
        ...
        self.mapping[ods_id] = (_type, uuid_str, in_collection)
        self._save_mapping()
        return True
    ...
    def get_all_entries(self) -> Dict[str, Tuple[str, str, Optional[str]]]:
        ...
        Returns:
            Dict[str, Tuple[str, str, Optional[str]]]: Dictionary of all ODS ID to (_type, UUID, inCollection) mappings
        """
